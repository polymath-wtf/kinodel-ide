...
Context7: Supabase password identifiers, LangGraph time travel/replay/interrupts и Pydantic v2 boundary validation. Primary web pages: Python venv/fcntl/msvcrt, Linux flock, Windows LockFileEx, SQLite PRAGMA (нужные busy/FK/version/integrity sections), GCS signed URLs/lifecycle. Дата обращения 2026-09-09; ссылки в новых local-startup/physical-dtos/rework и обновлённых identity/credits pages. Это не проверка реального deployment, аккаунтов, bucket/IAM, provider terms, auth login или pinned saver integration.

Добавлены три backend proposal pages: local-startup, physical-dtos, rework. Синхронизированы runtime/artifacts/reviews/tools/pipeline/implementation и database/roadmap. Прежние sketch DTO помечены non-authoritative, не выданы за совместимый shipped API. Supabase users/GCS/365-day outputs/MVP цены и signup grant отражены как требования пользователя; stdlib lock API, 15-minute URL TTL, terminal-only source и settlement outcomes как предложения. Backup/RPO/RTO/payment/subscription и подробный UX отложены, restart durability и authorization не отложены.
...
Ниже сохранён **предыдущий проход до принятия engine/rework решений**, не текущий статус. Его «предложено/не выбрано» заменено принятыми ответами в [реестре](open-questions.md#уже-отвечено-в-контрактах).
...
Исправления и severity-first выводы: [аудит и решения](audit-and-decisions.md). Новые сценарии отмечены #todo, а не пройденными tests. Локальные Markdown targets/anchors проверяются для database и изменённого proposal; diff whitespace проверяется отдельно для tracked и нового файла.
