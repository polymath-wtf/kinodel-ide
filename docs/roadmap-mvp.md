...
**Шаги 0–1 закрыты на Windows; шаг 2 в работе.** Внутренний runner и localhost API провели start → Story v1 → revise → v2 → approve через reopen; retry/cancel и остановка ожидающего owner call проверены на реальном saver. Далее — discussion и процессная приёмка. Экран пока отсутствует. Подробнее — [шаг 2](#step-2). Первый deploy — локальный Windows-пилот, не hosted-сервис.
...
- [x] **Первый localhost-прототип — API.** `backend/api.py` принимает internal test start, respond (`approve/revise`), retry/cancel; читает состояние, work, точный review и immutable версии Story. Lifespan владеет root/DB/saver и единственным polling runner; handlers сохраняют команды без вызова графа. Loopback peer, Host, Origin, сессионная cookie и CSRF действуют на HTTP, clarify отклоняется до записи decision/work. `tests/test_api.py` проходит start → revise → approve и чтение после restart, retry/cancel через reopen, доступность blocked execution и проверки границ HTTP. Команда запуска, session bootstrap и URL документации — в [local startup](backend/local-startup.md#internal-story-api-prototype). Это прототип с заменой модели, не закрытие шага 2; экран автора остаётся в шаге 6.
...
Последний общий прогон на Windows: `.\.venv313\Scripts\python.exe -m unittest discover -s tests -q` — **109 тестов, 4 пропуска** (нет Windows symlink privilege). Runner regression использует reopen и fault-injection; process-death/power-loss проверка остаётся открытой.
