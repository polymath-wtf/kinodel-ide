...
1. Pin the supplied product-default version and registered capability/profile versions in the Brief operation. Producer extracts explicit requirements, including provider preference; absent settings may use only these supplied defaults.
...
4. Validate image/video compatibility together for every required stage role, including main-frame generation, reference-conditioned story frames, image dimensions, and the start-image input required for `i2v`. A profile may bind different versioned workflows for these roles; one workflow per role is sufficient, not another registry. A text-only image workflow cannot fulfill a required image-anchor input by dropping it or substituting its textual description. No match, unsupported explicit requirements, or missing capability metadata fail closed before Brief review/provider submission. Use the existing bounded input path only for a choice the creator can resolve; never silently fall back to another provider or workflow class.
