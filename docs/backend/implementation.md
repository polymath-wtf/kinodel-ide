...
4. Prove text-only Brief/Story review/repair as an internal runtime milestone. Deployable `foundation.v0` follows [foundation routes](reviews.md#foundation-routes): Wardrobe anchor plan, sequential dependent rendering, complete `main_frames` review/save, then Storyboard shot planning, rendering and frame review/save. Activate the five capabilities through common contracts, not agent-name runtime branches. Keep text tests under a separate graph identity; never append rendering to an open thread's factory.
...
8. Implement anchor/shot DTOs/resources, direct media projections, minimal Q13 jobs/candidates, typed workflow mappings, external wait and approved-selection save inside gate apply. Prove portrait -> sheet -> independent location, local dependency-aware regeneration, and a shot using all required anchors. Pass lost-response, fast-result, duplicate-wake, invalid-import and late-cancel checks. Video/montage/memory, parallel shot execution and [rework entry](rework.md) remain later.
