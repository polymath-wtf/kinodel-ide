# Full documentation about Google Gemini Embeddings .
Это большая документация от google про Rag и их embedding модели, включая depricated `gemini-embedding-001`. 
Сейчас актуальна только `gemini-embedding-2` , другая модель нас не интересует.
и код для разных языков нам тоже не нужен, нам хватит только Python.
