...
- `references/final-reviewgate-ux.md` — p12 final-gate preview shape: show `MEDIA:/absolute/path/to/final.mp4` plus a concise `final_chunk.json` summary, then stop for A/B/C/D.
...
If the user responds with a terse continuation cue like `go`, `го`, `continue`, `дальше`, or `next` after a shown card, treat it as permission to advance to the next deterministic step; do not add a soft confirmation question.
