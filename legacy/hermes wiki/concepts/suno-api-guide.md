---
title: Suno provider music generation docs 
type: concept 
tags: [muse, kinodel, suno, music, provider, patch, music-video,music-gen, render]
status: reference-for-phase-e. Used to shape provider-neutral `music_request.json`; Muse plans requests, Render owns Suno provider calls.
---

Гайд по генерации музыки через provider Suno API. Нужно будет чтобы `muse-kinodel` генерировал готовый provider-neutral `music_request.json`, который затем передаётся `render-kinodel`.
