---
name: comfyui
description: Backup-only ComfyUI provider skill for Kinodel/local generation. Load
  only when the user explicitly requests ComfyUI, a render request provider is local-comfyui/comfyui,
  or fal.ai is unavailable and the user accepts backup rendering.
license: MIT
metadata:
  hermes:
    tags:
    ...
    prerequisites:
      ...
      - python3
    setup:
      help: Run scripts/hardware_check.py FIRST to decide local vs Comfy Cloud; then
        scripts/comfyui_setup.sh auto-installs locally (or use Cloud API key for platform.comfy.org).
    trigger: explicit ComfyUI provider use, provider=local-comfyui, provider=comfyui,
      ComfyUI install/setup/debug only
    hot_path: never load during normal Kinodel fal.ai production; backup/debug only
---
...
For Kinodel production, this skill is a provider toolkit/workflow runtime, not the render owner and not a production subagent. `render-kinodel` owns render scheduling, preflight, result/event artifacts, provider profiles, and manifest promotion. Use this skill for ComfyUI setup, health/dependency checks, workflow-format debugging, and runner utilities. Normal Kinodel render loops should call/import the ComfyUI runner through `render-kinodel`, not spawn an LLM `comfyui` subagent.

### Kinodel runner integration

For Kinodel production, do not shell out from Render to `run_workflow.py` unless maintaining a legacy path. Prefer importing `ComfyRunner`, `load_schema`, `inject_params`, and `download_outputs` from `scripts/run_workflow.py` inside `render-kinodel` provider adapters. This keeps one worker process responsible for structured errors, retries, output selection, and result/event manifests while still reusing the ComfyUI toolkit layer.
...
Host resolution note: the helper scripts default to `COMFYUI_LOCAL_NGROK_URL` when it is set (including from `~/.hermes/.env`), then fall back to `http://127.0.0.1:8188`. Use this for local ComfyUI exposed via ngrok without passing `--host` every time.
