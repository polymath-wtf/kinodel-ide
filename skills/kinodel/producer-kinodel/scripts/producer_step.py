...
- render_stage: launch render-kinodel/scripts/render.py in background with notify_on_complete=true.
...
COPY_WORKER_RESULT = RENDER_SKILL_DIR / "scripts" / "copy_worker_result.py"
...
def build_render_action(project_dir: Path, route: dict[str, Any], goal: str) -> dict[str, Any]:
    ...
    wakeup_argv = [
        "python3",
        str(COPY_WORKER_RESULT),
        "--project-dir",
        str(project_dir),
        "--worker-result",
        str(worker_result),
        "--stage",
        str(stage.get("result_artifact") or result_artifact).split("/")[-1].replace("_result.json", ""),
    ]
    return {
        "schema": "kinodel.producer_step.v1",
        "action": "render_stage",
        ...
        "launch_mode": "background_notify_on_complete",
        "wakeup": {
            "copy_command_argv": wakeup_argv,
            "copy_command": shell_join(wakeup_argv),
