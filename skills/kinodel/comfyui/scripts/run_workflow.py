...
class ComfyRunner:
    ...
    def local_queue_state(self, prompt_id: str) -> dict:
        """Return compact local ComfyUI queue state for one prompt.

        Local ComfyUI exposes queued/running prompts via /queue while /history/{id}
        is empty until terminal completion. Queue wait must not be counted as
        execution time; otherwise a prompt submitted behind a long video render can
        time out and be retried, creating duplicate queued renders.
        """
        if self.is_cloud:
            return {"state": "unknown"}
        r = http_get(self._url("/queue"), headers=self.headers, retries=1, timeout=30)
        if r.status != 200:
            return {"state": "unknown", "http_status": r.status, "body": r.text()[:300]}
        try:
            data = r.json() or {}
        except Exception:
            return {"state": "unknown", "body": r.text()[:300]}
        ...
        for item in running:
            if isinstance(item, list) and len(item) >= 2 and item[1] == prompt_id:
                return {"state": "running", "queue_running": len(running), "queue_pending": len(pending)}
        ...
        return {"state": "absent", "queue_running": len(running), "queue_pending": len(pending)}

    # ---------- HTTP polling ----------
    def poll_status(self, prompt_id: str, *, timeout: float = 300.0,
                    initial_interval: float = 1.5, max_interval: float = 8.0) -> dict:
        start = time.time()
        local_running_start: float | None = None
        local_last_queue_state: dict = {"state": "unknown"}
        ...
        while time.time() - start < timeout:
            if self.is_cloud:
                ...
                # Not in history yet. Check /queue so pending queue wait does not
                # burn the execution timeout. Only the actual running period is
                # bounded by `timeout`; queued prompts can wait behind older renders.
                q = self.local_queue_state(prompt_id)
                local_last_queue_state = q
                q_state = q.get("state")
                now = time.time()
                if q_state == "pending":
                    # Keep the outer loop alive while ComfyUI still owns the prompt
                    # but has not started executing it yet.
                    start = now
                    local_running_start = None
                elif q_state == "running":
                    if local_running_start is None:
                        local_running_start = now
                    # Convert the outer timeout into an execution-runtime timeout.
                    start = local_running_start

            time.sleep(interval)
            interval = min(max_interval, interval * 1.4)

        return {"status": "timeout", "elapsed": time.time() - start, "queue": local_last_queue_state}
...
def inject_params(
    ...
    *, randomize_seed_if_unset: bool = False,
    ...
    def set_param_target(name: str, value: Any, nid: str, m: dict) -> None:
        node = wf.get(nid)
        ...
        if "field_path" in m:
            ...
            if not isinstance(path, list) or not path:
                warnings.append(f"parameter '{name}' has invalid field_path for node '{nid}'")
                return
            cur: Any = node["inputs"]
            for key in path[:-1]:
                if not isinstance(cur, dict) or key not in cur:
                    warnings.append(f"parameter '{name}' path missing at {nid}.{'.'.join(map(str, path))}")
                    return
                cur = cur[key]
            leaf = path[-1]
            if isinstance(cur, dict):
                ...
                if is_link(existing):
                    warnings.append(
                        f"parameter '{name}' targets {nid}.{'.'.join(map(str, path))} which is currently a link; "
                        f"refusing to overwrite"
                    )
                    return
                cur[leaf] = value
                return
            warnings.append(f"parameter '{name}' parent path is not an object at node '{nid}'")
            return

        field = m.get("field")
        if not field:
            warnings.append(f"parameter '{name}' missing field/field_path in schema")
            return
    ...
    for name, value in args.items():
        ...
        if "node_ids" in m:
            for nid in m.get("node_ids") or []:
                set_param_target(name, value, str(nid), m)
        elif "node_id" in m:
            set_param_target(name, value, str(m["node_id"]), m)
        else:
            warnings.append(f"parameter '{name}' missing node_id/node_ids in schema")
