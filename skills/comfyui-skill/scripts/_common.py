...
def _read_dotenv_value(name: str) -> str | None:
    """Read a simple KEY=value from ~/.hermes/.env when the process env lacks it."""
...
ENV_LOCAL_NGROK_URL = "COMFYUI_LOCAL_NGROK_URL"
DEFAULT_LOCAL_HOST = _env_value(ENV_LOCAL_NGROK_URL) or "http://127.0.0.1:8188"
...
def resolve_api_key(explicit: str | None) -> str | None:
    """Look up API key from CLI flag → env var / ~/.hermes/.env. Strips whitespace and quotes."""
    val = explicit if explicit else _env_value(ENV_API_KEY)
    if val is None:
        return None
    return val or None
